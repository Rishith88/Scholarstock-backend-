...
router.get('/decks', verifyToken, async (req, res) => {
  try {
    ...
    const dueCounts = await Flashcard.aggregate([
      { $match: { userId: req.userId, dueDate: { $lte: now } } },
      { $group: { _id: '$deckId', count: { $sum: 1 } } },
    ]);
    const dueMap = {};
    dueCounts.forEach(d => { dueMap[d._id.toString()] = d.count; });
    const result = decks.map(d => ({ ...d.toObject(), dueCount: dueMap[d._id.toString()] || 0 }));
...
router.put('/decks/:id', verifyToken, async (req, res) => {
  try {
    const deck = await FlashcardDeck.findOneAndUpdate({ _id: req.params.id, userId: req.userId }, { $set: req.body }, { new: true });
...
router.post('/bulk', verifyToken, async (req, res) => {
  try {
    ...
    if (!deckId || !Array.isArray(cards) || cards.length === 0) return res.status(400).json({ success: false, message: 'deckId and cards array required' });
    const deck = await FlashcardDeck.findOne({ _id: deckId, userId: req.userId });
    if (!deck) return res.status(404).json({ success: false, message: 'Deck not found' });
    const toInsert = cards.filter(c => c.front && c.back).map(c => ({ userId: req.userId, deckId, front: c.front, back: c.back, hint: c.hint || '', tags: c.tags || [] }));
...
router.put('/:id', verifyToken, async (req, res) => {
  try {
    const card = await Flashcard.findOneAndUpdate({ _id: req.params.id, userId: req.userId }, { $set: { front: req.body.front, back: req.body.back, hint: req.body.hint, tags: req.body.tags } }, { new: true });
...
router.post('/:id/review', verifyToken, async (req, res) => {
  try {
    ...
    if (quality === undefined || quality < 0 || quality > 5) return res.status(400).json({ success: false, message: 'quality must be 0-5' });
...
router.get('/stats', verifyToken, async (req, res) => {
  try {
    ...
    const [totalCards, dueCards, deckCount, reviewStats] = await Promise.all([
      ...
      Flashcard.aggregate([{ $match: { userId: req.userId, totalReviews: { $gt: 0 } } }, { $group: { _id: null, totalReviews: { $sum: '$totalReviews' }, totalCorrect: { $sum: '$correctCount' } } }]),
    ]);
